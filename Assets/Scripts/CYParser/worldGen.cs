...
public class worldGen : MonoBehaviour {
	...
	IEnumerator WaitForRequest(WWW www) {
		...
		json.GetField ("plat", delegate(JSONObject plat) {
			foreach(JSONObject obj in plat.list) {
				...
				if (obj.list.Count == 6) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, obj[4].n, obj[5].n, 0);
				}
				if (obj.list.Count == 5) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, 1, obj[4].n, 0);
				}
				if (obj.list.Count == 4) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, "5", 1, obj[3].n, 0);
				}
			}
		});
		...
		json.GetField ("ramp", delegate(JSONObject ramps) {
			foreach(JSONObject obj in ramps.list) {
				string tex1;
				if (obj[3].type == JSONObject.Type.NUMBER) { tex1 = obj[3].n.ToString(); } else { tex1 = obj[3].str; }

				if (obj.list.Count == 4) {
					createRamp(obj[0].n, obj[1].n, "5", obj[2].n, obj[3].n);
				}

				if (obj.list.Count == 5) {
					createRamp(obj[0].n, obj[1].n, tex1, obj[2].n, obj[4].n);
				}
			}
		});

		json.GetField ("diaplat", delegate(JSONObject diaplat) {
			foreach(JSONObject obj in diaplat.list) {
				string tex1;
				if (obj[3].type == JSONObject.Type.NUMBER) { tex1 = obj[3].n.ToString(); } else { tex1 = obj[3].str; }
				
				if (obj.list.Count == 6) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, obj[4].n, obj[5].n, 1);
				}
				if (obj.list.Count == 5) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, 1, obj[4].n, 1);
				}
				if (obj.list.Count == 4) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, "5", 1, obj[3].n, 1);
				}
			}
		}, delegate(string name) {
			Debug.LogWarning("No Diagonal Plats");
		});

		json.GetField ("triwall", delegate(JSONObject triwalls) {
			foreach(JSONObject obj in triwalls.list) {
				string tex1;
				if (obj[3].type == JSONObject.Type.NUMBER) { tex1 = obj[3].n.ToString(); } else { tex1 = obj[3].str; }
				
				if (obj.list.Count == 6) {
					createTriwall(obj[0].n, obj[1].n, obj[2].n, tex1, obj[4].n, obj[5].n);
				}
			}
		}, delegate(string name) {
			Debug.LogWarning("No Triagonal Walls Found");
		});

		json.GetField ("triplat", delegate(JSONObject triplat) {
			foreach(JSONObject obj in triplat.list) {
				string tex1;
				if (obj[3].type == JSONObject.Type.NUMBER) { tex1 = obj[3].n.ToString(); } else { tex1 = obj[3].str; }
				
				if (obj.list.Count == 7) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, obj[5].n, obj[6].n, 0, obj[4].n);
				}
				if (obj.list.Count == 6) {
					createPlat(obj[0].n, obj[1].n, obj[2].n, tex1, 1, obj[5].n, 0, obj[4].n);
				}
			}
		}, delegate(string name) {
			Debug.LogWarning("No Triagonal Plats");
		});
	...
	void createPlat (float x, float y, float size, string tex, float zidx, float level, float dia, float tri = 0) {
		...
		gen_plat.GetComponent<generatePlat> ().dia = dia;
		gen_plat.GetComponent<generatePlat> ().tri = tri;
	}
	...
	void createRamp (float x, float y, string tex, float direction, float level) {
		GameObject gen_ramp = (GameObject)Instantiate (ramp);
		gen_ramp.GetComponent<generateRamp> ().x = -x;
		gen_ramp.GetComponent<generateRamp> ().y = y;
		gen_ramp.GetComponent<generateRamp> ().tex = tex;
		gen_ramp.GetComponent<generateRamp> ().direction = direction;
		gen_ramp.GetComponent<generateRamp> ().level = level;
	}

	void createTriwall (float x, float y, float flip, string tex, float dir, float level) {
		GameObject gen_triw = (GameObject)Instantiate (triwall);
		gen_triw.GetComponent<generateTriwall> ().x = -x;
		gen_triw.GetComponent<generateTriwall> ().y = y;
		gen_triw.GetComponent<generateTriwall> ().flip = flip;
		gen_triw.GetComponent<generateTriwall> ().tex = tex;
		gen_triw.GetComponent<generateTriwall> ().dir = dir;
		gen_triw.GetComponent<generateTriwall> ().level = level;
	}
